...
def predict(
    ...
    # Ensure consistency between input and model precision
    if fp16:
        if torch.cuda.is_available():  # Only use half precision on CUDA
            df = df.half()  # Convert input data to half precision if using fp16
        else:
            print("fp16 requested, but CUDA not available. Defaulting to float32.")
            df = df.float()
    else:
        df = df.float()  # Convert input data to float32 if not using fp16
